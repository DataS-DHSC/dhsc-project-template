...
def load_and_process() -> pd.DataFrame:
    ...
    summary_df = load_summary_data(DATA_DIR)
    nims_df  = load_nims_df(DATA_DIR)
...
def load_summary_data(data_path: Path) -> pd.DataFrame:
    ...
    """
    summary_df = pd.read_excel(data_path / 'summary.xlsx',
                               sheet_name = 'Table 2a')
    
    #define start and end
    #can jsut use drop null w/ 50% threshold instead 
    start_row = util.get_regex_match(summary_df.iloc[:,0],
                                '^Table 2a')
    end_row = util.get_regex_match(summary_df.iloc[:,0],
                            '^Source: NHS England$')
    summary_df = summary_df.iloc[start_row:end_row,:].reset_index(drop= True)

    final_col_names = [summary_df.iloc[2, x]
                      if summary_df.iloc[2, x] not in ['-', np.nan] 
                      else  summary_df.iloc[3, x] 
                      for x in range(0, len(summary_df.columns))]
    summary_df.columns = [x.lower().replace(' ','_')
                          for x in final_col_names]
    
    drop_null_cols = [x for x in summary_df.columns
                      if summary_df[x].isna().sum() > len(summary_df[x])*0.5]
    drop_null_rows = [x for x in summary_df.index
                      if summary_df.loc[x].isna().sum() > len(summary_df.loc[x])*0.5]
    summary_df = summary_df.drop([2,3] + drop_null_rows,
                                 axis =0)
    summary_df = summary_df.drop(drop_null_cols,
                                 axis =1)
    
    summary_df = summary_df.rename(columns = {'date':'weekday',
                                              'unknown1': 'unknown'})
    summary_df = summary_df.astype({'weekday': 'object',
...
def load_nims_df(data_path: Path) -> pd.DataFrame:
    ...
    nims_df.columns = [x.lower().replace(' ','_')
                            for x in nims_df.columns]
    nims_df = nims_df.astype({'date': 'datetime64[ns]',
                              'type': 'object',
...
def get_monthly_totals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped_df.loc[:,'month']  = pd.to_datetime(grouped_df[['year',
                                                            'month']].assign(DAY=1)).dt.month_name().str.slice(stop=3)
